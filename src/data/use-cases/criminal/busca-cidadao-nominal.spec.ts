...
import { BuscaCidadaoNominalCriminal } from "./busca-cidadao-nominal"

describe('Use Case - Busca Cidadao Nominal (Criminal)', () => {
  let repository: MockProxy<Repository>
  let sut: BuscaCidadaoNominalCriminal
  ...
  beforeEach(() => {
    repository = mock()
    sut = new BuscaCidadaoNominalCriminal(repository)
  })

  it('Deve retornar um resultado válido passando um nome valido', async () => {
    repository.buscaCidadaoNominalCriminal.mockResolvedValue(resultRepo)
  ...
  //   repository.buscaCidadaoNominalCriminal.mockResolvedValue([])
  ...
  //   repository.buscaCidadaoNominalCriminal.mockRejectedValue(error)
