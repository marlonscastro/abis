...
import { BuscaCidadaoNominalParametrizada } from "./busca-cidadao-nominal-parametrizada"

describe('Use Case - Busca Cidadao Nominal (Civil) Parametrizada', () => {
  let repository: MockProxy<Repository>
  let sut: BuscaCidadaoNominalParametrizada
  ...
  beforeEach(() => {
    repository = mock()
    sut = new BuscaCidadaoNominalParametrizada(repository)
  })

  it('Deve retornar um resultado válido passando um nome valido', async () => {
    repository.buscaCidadaoNominalParametrizada.mockResolvedValue(resultRepo)
  ...
  it('Deve retornar array vazio quando passar um Nome invalido ou Não existente', async () => {
    repository.buscaCidadaoNominalParametrizada.mockResolvedValue([])
  ...
  it('Deve lançar uma exceção ao ocorrer erro no repositório', async () => {
    repository.buscaCidadaoNominalParametrizada.mockRejectedValue(error)
